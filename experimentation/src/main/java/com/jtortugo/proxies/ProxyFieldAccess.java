...
public class ProxyFieldAccess {
	private static final ArrayList<ExperimentConfig> experimetConfigs = new ArrayList<>(List.of(
		new ExperimentConfig("read_after_read",
			...
			""")
			.addProxyConfig("DirectAccess_POJO", 			i -> { return new DirectAccess_POJO(i); 	})
			.addProxyConfig("Map_ProxyObject", 				i -> { return new Map_ProxyObject(i); 		})
			.addProxyConfig("Map_POJO", 					i -> { return new Map_POJO(i); 				})
			.addProxyConfig("Map_DatapathProxy", 			i -> { return new DatapathProxy(i); 	}),
		...
		new ExperimentConfig("ForLoop_Multiply_Add_Four_Read_Fields", 
            ...
            """)
			.addProxyConfig("DirectAccess_POJO", 			i -> { return new DirectAccess_POJO(i,i+1,i+2,i+3); 	})
			.addProxyConfig("Map_ProxyObject", 				i -> { return new Map_ProxyObject(i,i+1,i+2,i+3); 		})
			.addProxyConfig("Map_POJO", 					i -> { return new Map_POJO(i,i+1,i+2,i+3); 				})
			.addProxyConfig("Map_DatapathProxy", 			i -> { return new DatapathProxy(i,i+1,i+2,i+3); 	}),
...
//
    ...
    public static void main(String[] args) throws Exception {
    	String sourceName = args.length > 0 ? args[0] : null;
    	String configName = args.length > 1 ? args[1] : null;

    	for (var expConfig : experimetConfigs) {
    		if (sourceName != null && !sourceName.equalsIgnoreCase(expConfig.name)) {
    			continue;
    		}

    		System.out.println("Source: " + expConfig.name);
    		for (var proxyConfig : expConfig.proxyConfigs) {
    			if (configName != null && !configName.equalsIgnoreCase(proxyConfig.name)) {
    				continue;
    			}
